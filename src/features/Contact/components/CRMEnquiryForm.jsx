...
const CRMEnquiryForm = () => {
  ...
  return (
    <div className="bg-white p-5 sm:p-8 md:p-10 rounded-[24px] sm:rounded-[40px] shadow-[0_10px_40px_rgba(0,0,0,0.05)] border border-gray-100 max-w-2xl mx-auto relative overflow-hidden w-full">
      ...
      <p className="text-gray-500 text-[13px] sm:text-sm mb-6 sm:mb-8">Fill out the details below, and our team will get back to you within 24 hours.</p>
      ...
      {submitStatus === 'success' ? (
        ...
        >
          ...
          <p className="text-[13px] sm:text-sm">We have your details and will be in touch shortly.</p>
        </motion.div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 relative z-10 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 w-full">
            <div className="w-full">
              ...
              <input type="text" name="name" required value={formData.name} onChange={handleChange} className="w-full px-4 sm:px-5 py-3 rounded-[12px] sm:rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#4f46e5] focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-[16px] sm:text-[15px]" placeholder="Jane Doe" />
            </div>
            <div className="w-full">
              ...
              <input type="tel" name="phone" required value={formData.phone} onChange={handleChange} className="w-full px-4 sm:px-5 py-3 rounded-[12px] sm:rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#4f46e5] focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-[16px] sm:text-[15px]" placeholder="+91 98765 43210" />
            </div>
          </div>
          ...
          <div className="w-full">
            ...
            <input type="email" name="email" required value={formData.email} onChange={handleChange} className="w-full px-4 sm:px-5 py-3 rounded-[12px] sm:rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#4f46e5] focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-[16px] sm:text-[15px]" placeholder="jane@example.com" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 w-full">
            <div className="w-full">
              ...
              {/* text-[16px] is required on mobile to trigger the native scrolling wheel! */}
              <select name="eventType" value={formData.eventType} onChange={handleChange} className="w-full px-4 sm:px-5 py-3 rounded-[12px] sm:rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#4f46e5] focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-[16px] sm:text-[15px] text-gray-600 cursor-pointer">
            ...
            <div className="w-full">
              ...
              <select name="budget" value={formData.budget} onChange={handleChange} className="w-full px-4 sm:px-5 py-3 rounded-[12px] sm:rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#4f46e5] focus:ring-2 focus:ring-indigo-100 outline-none transition-all text-[16px] sm:text-[15px] text-gray-600 cursor-pointer">
          ...
          <div className="w-full">
            ...
            <textarea name="message" rows="4" value={formData.message} onChange={handleChange} className="w-full px-4 sm:px-5 py-3 rounded-[12px] sm:rounded-2xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-[#4f46e5] focus:ring-2 focus:ring-indigo-100 outline-none transition-all resize-none text-[16px] sm:text-[15px]" placeholder="Dates, location, child's interests, or business objectives..."></textarea>
          </div>

          <button type="submit" disabled={isSubmitting} className="w-full bg-[#4f46e5] hover:bg-pink-500 text-white font-bold py-3.5 sm:py-4 rounded-[12px] sm:rounded-2xl transition-all shadow-md hover:shadow-lg disabled:opacity-70 disabled:cursor-not-allowed text-[15px] mt-2">
            {isSubmitting ? 'Sending...' : 'Submit Enquiry'}
          </button>
          
          {submitStatus === 'error' && (
            <p className="text-red-500 text-[13px] sm:text-sm text-center mt-3 font-medium">Something went wrong. Please try again.</p>
          )}
        </form>
      )}
    </div>
